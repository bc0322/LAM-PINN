...
from lam_pinn.engine.checkpoints import (
    load_legacy_warm_start,
    save_checkpoint,
    save_legacy_split_weights,
)
...
def _train_backward_stage(
    ...
    for epoch in range(config.optimization.backward_stage_epochs):
        ...
        for _ in range(cycle_size):
            active_idx = cluster_to_index[int(repeated_row["Cluster"])]
            model.set_gate_pattern(active_idx=active_idx, active_value=1.0, inactive_value=0.1)
            properties = row_to_task_properties(repeated_row, config.physics)
            loss = task_loss(
                model,
                domain_points,
                bc1_y0,
        ...
        sum_loss.backward()
        optimizer.step()
        scheduler.step(float(sum_loss.item()))
        last_total_loss = float(sum_loss.item())
...
def meta_train(config: TrainConfig, config_path: str | Path | None = None) -> dict:
    ...
    if config.expected_num_clusters is not None and len(cluster_ids) != int(config.expected_num_clusters):
        raise ValueError(
            f"expected_num_clusters={config.expected_num_clusters}, but the CSV contains "
            f"{len(cluster_ids)} clusters: {cluster_ids}"
        )
    ...
    if config.warm_start_checkpoint_path:
        load_legacy_warm_start(model, config.warm_start_checkpoint_path, device=device)
        logger.info("loaded legacy warm start: %s", config.warm_start_checkpoint_path)
    ...
    for cycle_idx in range(num_cycles):
        ...
        if last_task_row is None:
            raise RuntimeError(f"Cycle {cycle_idx + 1} is empty.")

        backward_loss = _train_backward_stage(
            model=model,
            repeated_row=last_task_row,
        ...
        trace_records.append(
            {
                "cycle": cycle_idx + 1,
                "stage": "backward",
                "cluster": int(last_task_row["Cluster"]),
                "subnet_index": int(cluster_to_index[int(last_task_row["Cluster"])]),
                "E": float(last_task_row["E_raw"]),
                "f": float(last_task_row["f_raw"]),
                "k": float(last_task_row["k_raw"]),
    ...
    save_legacy_split_weights(run_dir / "checkpoints", model)
    ...
    summary = {
        ...
        "legacy_split_checkpoint_dir": str(run_dir / "checkpoints"),
